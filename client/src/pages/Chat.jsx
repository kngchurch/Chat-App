...
function Chat() {
  ...
  //   setIsLoading(false);
  // }
  const getContactsAndGroups = async () => {
    try {
      const usersResponse = await axios.get(
        `${allUsersRoute}/${currentUser._id}`
      );
      const groupsResponse = await axios.get(
        `${APP_HOST}/api/groups/user/${currentUser._id}`
      );

      const contactsData = usersResponse.data;
      const groupsData = groupsResponse.data.map((group) => ({
        ...group,
        isGroup: true, // Add a flag so you know this is a group
      }));

      setContacts([...groupsData, ...contactsData]); // Merge groups first then users
      setIsLoading(false);
    } catch (error) {
